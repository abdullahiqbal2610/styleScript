...
2. **Encodes** input text at the character level with noise modulation.
...
6. **Optimises** the full pipeline using a composite loss (style loss + content loss + quality loss).
...
| L_content | L2 / MSE loss between generated content and text indices |
...
The script runs one full training epoch and prints per-batch losses:

```
--- Starting StyleScript Training (1 Epoch) ---
Batch 1/5 | L_style: 0.4821 | L_content: 12.3456 | L_total: 13.1234
...
